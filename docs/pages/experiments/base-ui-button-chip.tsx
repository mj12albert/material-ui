...
import ChipButton from 'docs/data/material/components/chip-recipe/components/ChipButton';
import ChipLink from 'docs/data/material/components/chip-recipe/components/ChipLink';

export default function BaseUiButtonChipExperiment() {
    ...
    <ThemeProvider theme={theme}>
      ...
      <Box sx={{ p: 3, maxWidth: 720, bgcolor: 'background.default', color: 'text.primary' }}>
        ...
        <Section title="ChipButton — With icon">
          ...
          <ChipButton label="Small with icon" icon={<FaceIcon />} size="small" onClick={() => {}} />
        </Section>

        <Section title="ChipButton — With avatar">
          <ChipButton label="With avatar" avatar={<Avatar>M</Avatar>} onClick={() => {}} />
          <ChipButton
        ...
        <Section title="ChipButton — With delete">
          <ChipButton label="Deletable" onDelete={() => {}} onClick={() => {}} />
          <ChipButton
          ...
          <ChipButton
            ...
            onClick={() => {}}
          />
          <ChipButton label="Small deletable" size="small" onDelete={() => {}} onClick={() => {}} />
        </Section>

        <Section title="ChipButton — Delete only (no onClick)">
          <ChipButton label="Delete only" onDelete={() => {}} />
          <ChipButton label="With icon + delete" icon={<FaceIcon />} onDelete={() => {}} />
        </Section>
        ...
        <Section title="ChipLink — With delete">
          <ChipLink label="Deletable link" href="#chip-link" onDelete={() => {}} />
          <ChipLink
        ...
        <Section title="Live delete demo (with aria-live region)">
          ...
          >
            {chips.map((chip) => (
              <ChipButton key={chip} label={chip} onDelete={handleDelete(chip)} color="primary" />
            ))}
            {chips.length === 0 && (
