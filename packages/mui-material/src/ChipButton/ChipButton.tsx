...
import chipButtonClasses, {
  getChipButtonUtilityClass,
  ChipButtonClasses,
} from './chipButtonClasses';
 ...
 */
const ChipButton = React.forwardRef<HTMLDivElement, ChipButtonProps>(function ChipButton(
  inProps,
  ref,
) {
  ...
  const { type: topLevelType, ...otherWithoutType } = other as typeof other & { type?: (string) | undefined };
  ...
  if (!hasDelete) {
    return (
      <RootSlot as="button" {...rootProps} {...({ type: 'button' } as any) /* polymorphic: styled('div') rendered as <button> via `as` prop */}>
        {iconNode}
